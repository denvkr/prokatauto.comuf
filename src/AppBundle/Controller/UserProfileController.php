...
//echo 5;
	if (!isset($this->config))
        ...
        if ($request->get('user') && $request->get('password')){
                ...
                $defaults = array('login' => $retval[0],'password'=>$retval[1],'mail_address'=>$retval[2],'name'=>$retval[3],'last_name'=>$retval[4],'address'=>$retval[5],'age'=>$retval[6],'drivers_length'=>$retval[7],'rent_request'=>$retval[8],'captcha'=>null);
                //echo 6;
        ...
        if ($request->get('mail_link_activation')){
                ...
                $form = $formFactory->createBuilder('form',$defaults, array('action' => $action,'method' => 'POST'))
                        ...
                        ->add('drivers_length',TextType::class)
                        ->add('rent_request',TextareaType::class)
 ...
 $html = $this->container->get('templating')->render('user_profile.html.twig',array('form' => $form->createView()));//,'captcha'=>$CaptchaView
